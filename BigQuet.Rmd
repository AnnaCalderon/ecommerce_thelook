...
sales_by_quarter <- order_items %>%
  mutate(quarter = as_date(floor_date(created_at, "quarter"))) %>%
  group_by(quarter) %>%
  summarise(total_sales = sum(sale_price, na.rm = TRUE), .groups = "drop")
...
```{r}
# Join event data with order data and order_items to get sales and profit information
event_order_link <- events %>%
  filter(event_type == "purchase") %>%
  left_join(orders, by = "user_id") %>%
  filter(created_at.x >= created_at.y - days(1) & created_at.x <= created_at.y + days(1)) %>%
  left_join(order_items, by = "order_id") %>%
  left_join(products, by = c("product_id" = "id")) %>%  # Correct join based on product_id and id
  select(event_id = id.x, order_id, user_id.x, event_time = created_at.x, order_time = created_at.y,
         status.y, traffic_source, sale_price, num_of_item, cost)

# Summarize the conversion data by traffic source
conversion_by_traffic <- event_order_link %>%
  group_by(traffic_source) %>%
  summarise(total_events = n(),
            converted_orders = sum(status.y %in% c("Shipped", "Complete"), na.rm = TRUE),
            conversion_rate = converted_orders / total_events * 100,
            total_sales = sum(sale_price * num_of_item, na.rm = TRUE),
            total_profit = sum((sale_price - cost) * num_of_item, na.rm = TRUE),
            .groups = "drop")

# View the summarized conversion data
print(conversion_by_traffic)
